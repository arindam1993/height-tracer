...
#define TERRAIN_SCALE vec2(257.0 * 1.24739482778494, 257.0 * 1.24739482778494)
...
float sampleHeight(sampler2D heightmap, vec2 uv) {
  ...
  return (-10000.0 + (s.x * 255.0 * 256.0 * 256.0 + s.y * 255.0 * 256.0 + s.z * 255.0) * 0.1) * terrainScale;
}
...
bool raymarch(sampler2D heightmap, vec3 origin, vec3 dir, vec3 worldPos, vec3 normal, out vec3 refinedWorldPos) {
  ...
  for( int i = 1; i < MARCH_STEPS; i++){
    float heightOffset = mix(MAX_HEIGHT, MIN_HEIGHT, float(i)/float(MARCH_STEPS));
    ...
    if(currHeight > currWsPoint.z) {
      refinedWorldPos = vec3(currWsPoint.xy, currHeight);//interpolateBetweenMarches(prevWsPoint, prevHeight, currWsPoint, currHeight, normal);
      found = true;
      break;
    }else{
