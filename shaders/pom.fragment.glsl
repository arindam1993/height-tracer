...
float sampleHeight(sampler2D heightmap, vec2 uv) {
  ...
  return -10000.0 + (s.x * 255.0 * 256.0 * 256.0 + s.y * 255.0 * 256.0 + s.z * 255.0) * 0.1;
}

vec2 raytrace(sampler2D heightmap, vec3 origin, vec3 dir, vec3 P, vec3 N, float height) {
  vec2 wsInstersection = rayPlaneIntersect(origin, dir, P, N , height);
  //scale by tilesize
  vec2 uv = clamp(wsInstersection/TERRAIN_SCALE, 0.0, 1.0);
  uv.y = 1.0 - uv.y;
  float h = sampleHeight(heightmap, uv);
  return uv;
}

// bool raymarch(sampler2D heightmap, vec3 origin, vec3 dir, vec3 worldPos, vec3 normal, out refinedWorldPos, out refinedNormal) {
//   float currentHeight = 
...
void main() {
  ...
  vec3 fdx = vec3( dFdx( vViewPosition.x ), dFdx( vViewPosition.y ), dFdx( vViewPosition.z ) );
	...
	#endif
#endif
  vec2 gray = raytrace(heightmap, cameraPosition, rayDir, vWorldPosition, flatNormal, 0.0);
	vec4 diffuseColor = texture2D(heightmap, gray);//vec4( diffuse, opacity );
